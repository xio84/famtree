  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-2xl shadow-md p-10 w-full max-w-sm text-center">
        ...
        <p className="text-gray-500 mb-8 text-sm">Sign in to manage your family history</p>
        <form
          action={async () => {
            "use server"
        ...
        {isDev && (
          <form
            action={async () => {
              "use server"
              await signIn("dev", { redirectTo: "/tree" })
            }}
            className="mt-3"
          >
            <button
              type="submit"
              className="w-full rounded-lg border border-dashed border-amber-400 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900 hover:bg-amber-100 transition"
            >
              Sign in as Dev User
            </button>
            <p className="mt-2 text-xs text-gray-400">dev-only — not shown in production</p>
          </form>
        )}
      </div>
    </div>
